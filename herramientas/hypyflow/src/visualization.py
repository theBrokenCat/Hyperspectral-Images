    ...
    def mouse_callback(self, event, x, y, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN:  # Left mouse click
            ...
            if 10 <= x <= 160 and 10 <= y <= 50:
                ...
                self.spectrum = self.hsi[y, x, :] # esto es para obtener el valor de reflectancia de la banda en la que se hizo click
                print(f"Clicked pixel: {self.clicked_pixel_coords}, Spectral signature: {self.spectrum}")

                # Plot the spectral signature
                plt.figure()
                plt.plot(range(self.hsi.shape[2]), self.spectrum) # eje x es el rango de bandas y el eje y es el valor de reflectancia. self.spectrum es el valor de reflectancia de la banda en la que se hizo click
                # y self.hs.shape[2] es el numero de bandas
