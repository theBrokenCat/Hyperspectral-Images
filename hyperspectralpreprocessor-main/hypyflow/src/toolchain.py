...
class PreprocessingPipeline:
    ...
    def _analyze_data(self, data: np.ndarray) -> None:
        ...
        print(f" - Data mean: {mean_val} - Data std: {std_val}")
    ...
    def __call__(
        ...
        if previous_mask is not None:
            ...
            for block in masking_blocks:
                if verbose:
                    print(" - [MASK] Running pipeline block: " + str(block))
        ...
        # Instead of using multiple small logical operations, optimize full_mask combining
        full_mask = np.logical_and.reduce(masks, axis=0) if masks else np.ones(data.shape[:2], dtype=bool)

        # Now we are going to run the processing blocks
        for block in processing_blocks:
            if verbose:
                print(" - [PROCESS] Running pipeline block: " + str(block))
            if not isinstance(block, GenericPipelineBlock):
                raise TypeError("Pipeline element" + str(block) + " must be child of GenericPipelineBlock")
